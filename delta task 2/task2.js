 ...
 function coll_detect(){
   ...
   for(let i=0;i<k;i++){
    for(let j=0;j<enemies_arr.length;j++){
      ...
      if((Math.abs(Ox[i]-enemies_arr[j].x)<40)&&(Math.abs(Oy[i]-enemies_arr[j].y)<40)){
...
function bullet_move() {
  for(let i=0;i<k;i++){
        ...
        if((cx[i]-x_[i])>0){
          Oy[i]=y_[i]+z[i]*m[i]/Math.sqrt(1+m[i]*m[i]);
          Ox[i]=x_[i]+z[i]/Math.sqrt(1+m[i]*m[i]);
              bullet_arr.push(new bullets(Ox[i],Oy[i]));
              if(u[i]==i){
                Ox[i]=0;
                Oy[i]=0;
                stop_bull[i]=0;
        ...
        ctx.beginPath();
        ctx.arc(Ox[i],Oy[i] , 10, 0, 2 * Math.PI);
        ctx.fillStyle = 'red';
        ctx.fill();
